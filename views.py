...
def index(request):
    if request.session.has_key('is_loggged'):
        ...
        def addmoney_submission(request):
            if request.session.has_key('is_logged'):
                if request.method == "POST":
                    ...
                    addmoney_info1 = Addmoney_info.objects.filter(
                    ...
                    add = Addmoney_info(
                        user=user1, add_money=add_money, quantity=quantity, Date=Date, Category=Category)
                    add.save()
            ...
            def expense_edit(request, id):
                if request.session.has_key('is_logged'):
                    ...
                    user1 = User.objects.get(id=user_id)
                    return
                render(request, 'home/expense_edit.html',
